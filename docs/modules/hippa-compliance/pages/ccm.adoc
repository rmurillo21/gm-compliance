...
. Before provisioning any systems, a request must be created and approved in the {ticketing-system} Production Change Management (PRODCM) project.
 ** {ticketing-system} access requires authenticated users.
 ** Security grants access to the {ticketing-system} PRODCM project following the procedures covered in the xref:access.adoc[Access Establishment and Modification section].
...
----
SSO/IdP ── {company-short-name}-master
            │  └── billing and root DNS records only
            │
            ├── {company-short-name}-dev
            │    └── VPC
            │        └── Subnets
            │             └── Security-Groups
            │                  └── EC2 instances
...
Each new environment is created as a sub-account connected to `+{company-short-name}-master+`.
...
+
{hr-system-name} is used for continuous delivery (build and deploy), and we employ {hr-system-name}-{ticketing-system} automation such that:

 ** Whenever deployment to a controlled environment (e.g.
production accounts and infrastructure account) is requested, the {hr-system-name} job will check for an approved PRODCM ticket, or create a new ticket if not found.
 ** The automation code will attempt to automatically populate the required data for the PRODCM ticket (see list above).
 ** If the data cannot be automatically populated, {hr-system-name} may pause the job and prompt the user for manual input.
 ** Job will be paused until the request is approved or canceled (rejected).
Before continuing to deployment, {hr-system-name} will validate the change request's build job identifier, build number and source code branch.
 ...
 ** Whenever a PRODCM ticket is created, the bot is triggered via a {ticketing-system} webhook.
 ...
 ** Details of the analysis are posted to the PRODCM {ticketing-system} ticket.
...
{hr-system-name} job proceeds only with an approved and validated PRODCM ticket.
...
It can be created in the {ticketing-system-url}[Change Management System/Portal] which implements the {company-short-name} Change Management workflow, using the Production Change Management (PRODCM) {ticketing-system} project to manage changes and approvals.

* At least two approvals are required for each PRODCM ticket.
...
and
 ** the DevOps Lead is added as an approver for changes to `+aws-{company-short-name}-infra+` account in AWS.
...
`+aws-{company-short-name}-infra+`, `+aws-{company-short-name}-prod-us+`, `datacenter-hq`)
...
* *Post-emergency Documentation:* A PRODCM ticket should be created within 24 hours following the emergency change.
